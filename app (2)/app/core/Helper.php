...
/**
 * Display text safely - decodes HTML entities first to prevent double-encoding
 * Use this instead of htmlspecialchars when displaying data that might already be encoded
 * This fixes issues like apostrophes showing as &#039; instead of '
 */
function displayText($text) {
    if (empty($text)) {
        return '';
    }
    // Decode any existing HTML entities first (handles &#039;, &apos;, etc.)
    $decoded = html_entity_decode($text, ENT_QUOTES | ENT_HTML5, 'UTF-8');
    // Decode again in case of double-encoding
    $decoded = html_entity_decode($decoded, ENT_QUOTES | ENT_HTML5, 'UTF-8');
    // Then encode properly for safe display
