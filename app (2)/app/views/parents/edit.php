<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold">Edit Parent/Guardian</h1>
        <div class="space-x-2">
            <a href="<?php echo BASE_URL; ?>/parents/show/<?php echo $parent['id']; ?>" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">
    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form id="parentForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="first_name" required class="w-full border rounded px-3 py-2" 
                           value="<?php echo htmlspecialchars($parent['first_name']); ?>">
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Last Name *</label>
                    <input type="text" name="last_name" required class="w-full border rounded px-3 py-2" 
                           value="<?php echo htmlspecialchars($parent['last_name']); ?>">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" name="phone_alt" class="w-full border rounded px-3 py-2" 
                           value="<?php echo htmlspecialchars($parent['phone_alt'] ?? ''); ?>">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="id_number" class="w-full border rounded px-3 py-2" 
                           value="<?php echo htmlspecialchars($parent['id_number'] ?? ''); ?>">
                </div>
                ...
                <div>
                    ...
                    <select name="relationship" class="w-full border rounded px-3 py-2">
                        ...
                        <option value="guardian" <?php echo ($parent['relationship'] ?? 'guardian') == 'guardian' ? 'selected' : ''; ?>>Guardian</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="address" rows="3" class="w-full border rounded px-3 py-2"><?php echo htmlspecialchars($parent['address'] ?? ''); ?></textarea>
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
                    <select name="status" class="w-full border rounded px-3 py-2">
                        <option value="active" <?php echo ($parent['status'] ?? 'active') == 'active' ? 'selected' : ''; ?>>Active</option>
            ...
            <div class="mt-6 flex justify-end space-x-4">
                <a href="<?php echo BASE_URL; ?>/parents/show/<?php echo $parent['id']; ?>" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">
                    Cancel
                </a>
...
document.getElementById('parentForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo BASE_URL; ?>/parents/update/<?php echo $parent['id']; ?>', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            if (data.redirect) {
                ...
                window.location.href = '<?php echo BASE_URL; ?>/parents/show/<?php echo $parent['id']; ?>';
            }
        } else {
            let errorMsg = data.message || 'Failed to update parent';
            if (data.errors) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-save mr-2"></i>Update Parent';
        }
    } catch (error) {
