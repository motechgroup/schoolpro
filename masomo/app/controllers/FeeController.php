...
class FeeController extends Controller {
    ...
    public function paymentForm($studentId) {
        ...
        // Get fee heads with payment breakdown
    ...
    public function checkPaymentStatus() {
        ...
        try {
            ...
            // Strategy 4: Query M-Pesa API for transaction status (do this early, not as last resort)
            require_once APP_PATH . '/helpers/MpesaHelper.php';
            
            // If transaction exists but no receipt, query API immediately
            if ($transaction && empty($transaction['mpesa_receipt_number']) && empty($transaction['payment_id'])) {
                error_log("M-Pesa Payment: Transaction exists but no receipt - Querying API immediately");
                ...
                if ($queryResult['success'] && isset($queryResult['data'])) {
                    ...
                    if ($resultCode == 0) {
                        // Payment successful - extract receipt and update
                        $mpesaReceipt = '';
                        $amount = 0;
                        $transactionDate = '';
                        ...
                        if (!empty($mpesaReceipt) && $amount > 0) {
                            ...
                            $updateStmt->execute([$mpesaReceipt, $amount, $transactionDate, $checkoutRequestId]);
                            ...
                            error_log("M-Pesa Payment: Updated from API - Receipt: $mpesaReceipt, Amount: $amount");
            ...
            // Query M-Pesa API for transaction status (aggressive check)
            $queryResult = MpesaHelper::queryTransactionStatus($checkoutRequestId);
