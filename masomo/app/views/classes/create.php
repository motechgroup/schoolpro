    ...
    <div class="bg-white rounded-lg shadow p-6">
        <form id="classForm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="grade_id" required class="w-full border rounded px-3 py-2">
                        ...
                        <option value="<?php echo $grade['id']; ?>" <?php echo (isset($selectedGradeId) && $selectedGradeId == $grade['id']) ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <input type="text" name="name" required 
                           placeholder="e.g., PP1, Grade 1, or A, B for multiple classes" 
                           class="w-full border rounded px-3 py-2">
                    <p class="text-xs text-gray-500 mt-1">For most Kenyan schools, use the grade name (e.g., "PP1", "Grade 1"). Use A, B, etc. only if you have multiple classes per grade.</p>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="capacity" value="40" min="1" max="60"
                           class="w-full border rounded px-3 py-2">
            ...
            <div class="mt-6 flex justify-end space-x-4">
                <a href="<?php echo BASE_URL; ?>/classes" class="bg-gray-500 text-white px-6 py-2 rounded hover:bg-gray-600">
                    Cancel
                </a>
...
document.getElementById('classForm').addEventListener('submit', async function(e) {
    ...
    try {
        const response = await fetch('<?php echo BASE_URL; ?>/classes/store', {
            method: 'POST',
            body: formData
        });
        ...
        if (data.success) {
            window.location.href = data.redirect || '<?php echo BASE_URL; ?>/classes';
        } else {
