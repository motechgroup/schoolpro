<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow p-6 mt-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
                <p class="text-xs text-gray-500 uppercase font-semibold">Total Year Billed</p>
                <p class="text-xl font-bold text-blue-700"><?php echo formatCurrency($termSummary['total_billed'] ?? 0); ?></p>
            </div>
            <div class="bg-green-50 border-l-4 border-green-500 p-4 rounded">
                <p class="text-xs text-gray-500 uppercase font-semibold">Total Year Paid</p>
                <p class="text-xl font-bold text-green-700"><?php echo formatCurrency($termSummary['total_paid'] ?? 0); ?></p>
            </div>
            <div class="<?php echo ($termSummary['net_balance'] ?? 0) > 0 ? 'bg-red-50 border-l-4 border-red-500' : 'bg-emerald-50 border-l-4 border-emerald-500'; ?> p-4 rounded">
                <p class="text-xs text-gray-500 uppercase font-semibold">Net Cumulative Balance</p>
                <p class="text-xl font-bold <?php echo ($termSummary['net_balance'] ?? 0) > 0 ? 'text-red-700' : 'text-emerald-700'; ?>">
                    <?php 
                        $nb = $termSummary['net_balance'] ?? 0;
                        if ($nb < 0) {
                            echo formatCurrency(abs($nb)) . ' (Credit)';
                        } else {
                            echo formatCurrency($nb);
                        }
                    ?>
                </p>
            </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Term Fee</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Arrears/Credit In</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <?php 
                        $carriedIn = $invoice['carried_in'] ?? 0;
                        $totalPayable = $invoice['total_payable'] ?? $invoice['total_amount'];
                        $netTermBalance = $invoice['net_term_balance'] ?? $invoice['balance'];
                    ?>
                    <tr>
                        <td class="px-4 py-4 whitespace-nowrap text-sm font-medium">Term <?php echo $invoice['term']; ?></td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-600"><?php echo htmlspecialchars($invoice['invoice_number']); ?></td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-right"><?php echo formatCurrency($invoice['total_amount']); ?></td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-right font-medium <?php echo $carriedIn > 0 ? 'text-red-600' : ($carriedIn < 0 ? 'text-green-600' : 'text-gray-500'); ?>">
                            <?php 
                                if ($carriedIn > 0) {
                                    echo '+' . formatCurrency($carriedIn);
                                } elseif ($carriedIn < 0) {
                                    echo '-' . formatCurrency(abs($carriedIn)) . ' (Cr)';
                                } else {
                                    echo formatCurrency(0);
                                }
                            ?>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-right font-semibold text-gray-800"><?php echo formatCurrency($totalPayable); ?></td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-right text-green-600 font-medium"><?php echo formatCurrency($invoice['paid_amount'] ?? 0); ?></td>
                        <td class="px-4 py-4 whitespace-nowrap text-sm text-right font-bold <?php echo $netTermBalance > 0 ? 'text-red-600' : ($netTermBalance < 0 ? 'text-green-600' : 'text-gray-800'); ?>">
                            <?php 
                                if ($netTermBalance < 0) {
                                    echo formatCurrency(abs($netTermBalance)) . ' Cr';
                                } else {
                                    echo formatCurrency($netTermBalance);
                                }
                            ?>
                        </td>
                        ...
                        <?php else: ?>
                        <td class="px-4 py-4 whitespace-nowrap text-center text-sm text-gray-400">-</td>
                        <?php endif; ?>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
