            ...
            <div class="flex items-start space-x-6 mb-4">
                ...
                <div class="flex-1">
                    <h2 class="text-2xl font-bold mb-2">
                        <?php echo displayText($student['first_name'] . ' ' . ($student['middle_name'] ?? '') . ' ' . $student['last_name']); ?>
                    </h2>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <p class="text-lg"><?php echo displayText(($student['parent_first_name'] ?? '') . ' ' . ($student['parent_last_name'] ?? '')); ?></p>
                </div>
...
<!-- Payment Modal -->
<?php if (Auth::hasAnyRole(['super_admin', 'school_admin', 'bursar'])): ?>
<div id="paymentModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto w-full z-50" style="display: none; z-index: 9999;">
    <div class="flex items-center justify-center min-h-screen px-4 py-4">
        <div class="inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all w-full max-w-2xl my-auto">
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="flex justify-between items-center mb-4 sticky top-0 bg-white pb-2 border-b">
                    ...
                    <button onclick="closePaymentModal()" class="text-gray-400 hover:text-gray-600 focus:outline-none" type="button" aria-label="Close">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                <div id="paymentFormContainer" class="min-h-[200px] max-h-[calc(100vh-250px)] overflow-y-auto pr-2" style="min-height: 200px;">
                    <!-- Payment form will be loaded here -->
                    <div class="text-center py-4 text-gray-500">
                        <i class="fas fa-info-circle mr-2"></i>Loading payment form...
                    </div>
                </div>
            </div>
        </div>
...
function showPaymentForm(studentId, term, academicYear) {
    ...
    const fetchUrl = '<?php echo BASE_URL; ?>/fees/paymentForm/' + studentId + '?term=' + term + '&academic_year=' + encodeURIComponent(academicYear);
    console.log('Loading payment form from:', fetchUrl);
    ...
    fetch(fetchUrl)
        ...
        .then(html => {
            ...
            if (html.includes('<html') && !html.includes('paymentForm')) {
                console.warn('Response appears to be a full HTML page, not a partial form');
                throw new Error('Server returned full page instead of form. Check if authentication is working.');
            }
            ...
            // If form loaded successfully, scroll container to top
            container.scrollTop = 0;
            
            // Check if form actually loaded
            const form = container.querySelector('#paymentForm');
            if (!form) {
                console.warn('Payment form not found in loaded HTML');
                const errorHtml = '<div class="text-yellow-600 p-4 border border-yellow-300 rounded bg-yellow-50">' +
                    '<p class="font-semibold">⚠️ Warning: Payment form structure not found</p>' +
                    '<p class="text-sm mt-2">The server responded but the form HTML was not recognized.</p>' +
                    '<p class="text-xs mt-2 text-gray-600">Loaded content length: ' + html.length + ' characters</p>' +
                    '</div>';
        ...
        .catch(error => {
            ...
            const errorMsg = 'Error loading payment form: ' + error.message + '<br><small>URL: ' + fetchUrl + '</small><br><small>Please check the browser console for more details.</small>';
            container.innerHTML = '<div class="text-red-600 p-4 border border-red-300 rounded">' + errorMsg + '</div>';
            
            // Show error alert as well
            alert('Failed to load payment form. Error: ' + error.message + '\n\nPlease check:\n1. Your internet connection\n2. Server is running\n3. Browser console for details (F12)');
        });
}
...
// Close modal when clicking outside (set up after DOM is ready)
