<form id="paymentForm" action="#" method="POST" onsubmit="return false;">
    ...
    <?php 
        $carriedIn = 0;
        $totalPayable = floatval($invoice['balance']);
        if (!empty($termSummary['invoices'])) {
            foreach ($termSummary['invoices'] as $tInv) {
                if ($tInv['id'] == $invoice['id']) {
                    $carriedIn = $tInv['carried_in'] ?? 0;
                    $totalPayable = max(0, $tInv['net_term_balance'] ?? $invoice['balance']);
                    break;
                }
            }
        }
        $cumBalance = $cumulativeBalance ?? $totalPayable;
    ?>
    <div class="mb-4 bg-gray-50 border p-3 rounded space-y-1">
        <p class="text-sm text-gray-700">Student: <strong><?php echo htmlspecialchars($student['first_name'] . ' ' . $student['last_name']); ?></strong></p>
        <p class="text-sm text-gray-700">Term Invoice: <strong><?php echo htmlspecialchars($invoice['invoice_number']); ?> (Term <?php echo $invoice['term']; ?>)</strong></p>
        
        <div class="grid grid-cols-2 gap-2 mt-2 pt-2 border-t text-xs">
            <div>
                <span class="text-gray-500">Current Term Fee:</span>
                <span class="font-semibold text-gray-800"><?php echo formatCurrency($invoice['total_amount']); ?></span>
            </div>
            <div>
                <span class="text-gray-500">Prior Arrears/Credit:</span>
                <span class="font-semibold <?php echo $carriedIn > 0 ? 'text-red-600' : ($carriedIn < 0 ? 'text-green-600' : 'text-gray-600'); ?>">
                    <?php echo $carriedIn > 0 ? '+' . formatCurrency($carriedIn) : ($carriedIn < 0 ? '-' . formatCurrency(abs($carriedIn)) . ' (Cr)' : formatCurrency(0)); ?>
                </span>
            </div>
        </div>
        
        <div class="mt-2 pt-2 border-t flex justify-between items-center text-sm">
            <span class="font-semibold text-gray-700">Total Payable Balance:</span>
            <span class="font-bold text-base <?php echo $cumBalance > 0 ? 'text-red-600' : 'text-green-600'; ?>">
                <?php echo formatCurrency($cumBalance); ?>
            </span>
        </div>
    </div>
    ...
    <div class="space-y-4">
        <div>
            ...
            <input type="number" name="total_amount" id="totalAmount" required 
                   step="0.01" 
                   min="0.01"
                   max="<?php echo max($cumBalance, floatval($invoice['balance'])); ?>"
                   value=""
                   placeholder="Enter amount to pay (max: <?php echo formatCurrency(max($cumBalance, floatval($invoice['balance']))); ?>)"
                   class="w-full border rounded px-3 py-2 font-semibold">
            <p class="text-xs text-gray-500 mt-1">
                <span class="font-semibold">Total Balance Due: <?php echo formatCurrency($cumBalance); ?></span>
                <span class="ml-2 text-orange-600">⚠️ Enter actual amount paid</span>
            </p>
        </div>
