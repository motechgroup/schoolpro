<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold">KCB Bank Transactions</h1>
            <p class="text-gray-600 mt-1">Fetch and reconcile payments from KCB Bank via Buni API</p>
        </div>
    ...
    <div class="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-6">
        <strong>Configuration Required:</strong> Please configure KCB Bank account number and Buni API credentials in 
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h2 class="text-xl font-semibold text-blue-600">KCB Bank Account</h2>
                <p class="text-gray-600 mt-1">Account: <strong><?php echo htmlspecialchars($account_number); ?></strong></p>
            </div>
            <?php if ($balance !== null): ?>
            <div class="text-right">
                <p class="text-sm text-gray-600">Available Balance</p>
                <p class="text-2xl font-bold text-blue-600"><?php echo formatCurrency($balance); ?></p>
            </div>
            <?php endif; ?>
        </div>
    </div>
...
function fetchTransactions() {
    ...
    fetch('<?php echo BASE_URL; ?>/kcbbank/fetchTransactions', {
        method: 'POST',
        body: formData
    })
...
function reconcileTransaction(transactionId, studentId, amount, date, reference) {
    ...
    fetch('<?php echo BASE_URL; ?>/kcbbank/reconcile', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
